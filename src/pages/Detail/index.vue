<template>
  <div class="detail">
    ...
    <section class="con">
      ...
      <!-- 导航路径区域 -->
      <div class="conPoin">
        <!-- categoryView可能是空对象{},空对象的属性会是undefined。 -->
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr,index) in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <!-- 循环属性的值 -->
                <dd changepirce="0" :class="{active:spuValueAttr.isChecked =='1'}" v-for="(spuValueAttr,index) in spuSaleAttr.spuSaleAttrValueList" :key="spuValueAttr.id" @click="changeActivate(spuSaleAttr.spuSaleAttrValueList,spuValueAttr.id)">
                  {{spuValueAttr.saleAttrValueName}}
                </dd>
              </dl>
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 这里进行路由跳转之前，需要先发送请求，将购买的信息以请求通知服务器，服务器进行响应的存储。 -->
...
export default {
  ...
  computed: {
    ...
    // 给子组件Zoom的数据进行兜底，防止服务器的数据没有传递过来出现undefined
    skuImageList () {
      return this.skuInfo.skuImageList || [];
    }
  },
  methods: {
    // 设置产品售卖属性：颜色、版本等的属性高亮点击事件。
    changeActivate (SaleAttrList, id) {
      // 使用id对List中的对象进行排他操作
    ...
    // 表单元素修改产品数量
    changeSkuNum (event) {
      // 如何排除非数值类型的输入呢？ 使用  value * 1 的结果判断是否为 Nan、
    ...
    async addShopCar () {
      try {
        ...
        // 服务器存储成功---进行路由跳转（带参数：数量）
        this.$router.push({ name: 'addcartsuccess', query: { skuNum: this.skuNum } });
        // 而比较复杂的信息数据如：skuInfo信息，通过会话存储(不持久化，会话结束数据再消失)
